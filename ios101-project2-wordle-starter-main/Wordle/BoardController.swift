...
class BoardController: NSObject,
    ...
    // Tip: Take a look at how resetBoard is implemented above. The only difference is that you don't want to change the settings
     ...
     func resetBoardWithCurrentSettings() {
        // Reset numTimesGuessed to 0
           numTimesGuessed = 0
           
           // Reload the collection view to reset the board
           collectionView.reloadData()
        // END YOUR CODE HERE
    }
    ...
    private func applyNumGuessesSettings(with settings: [String: Any]) {
        // Tip 1: Use a breakpoint to inspect or print the `settings` argument
        // You can use print() or breakpoints to inspect the contents of the settings dictionary
        
        // Tip 2: There is a constant `kNumGuessesKey` in Constants.swift that you can use as the key to grab the value in the dictionary
        // Access the value associated with the kNumGuessesKey from the settings dictionary
        
        // Tip 3: Assign the correct value of the setting to the `numRows` property.
        // Cast the value associated with kNumGuessesKey to the correct type and assign it to numRows
        
        // Tip 4: You will need to cast the value to the correct type
        // Use optional binding or guard statement to safely unwrap and cast the value
        if let numGuesses = settings[kNumGuessesKey] as? Int {
            // Assign the value to numRows
            numRows = numGuesses
        }
    }
    ...
    private func applyThemeSettings(with settings: [String: Any]) {
        // Tip 1: Use optional binding to safely unwrap the theme string from the settings dictionary
        if let themeString = settings[kWordThemeKey] as? String,
            // Tip 2: Convert the theme string to WordTheme using the rawValue initializer
           let theme = WordTheme(rawValue: themeString) {
            // Pass the theme to WordGenerator.generateGoalWord and assign its result to the `goalWord`
            self.goalWord = WordGenerator.generateGoalWord(with: theme)
        }
    }
    ...
    private func applyIsAlienWordleSettings(with settings: [String: Any]) {
        // START YOUR CODE HERE
        // Tip 1: Use optional binding to safely unwrap the value from the settings dictionary
        if let isAlienWordleSetting = settings[kIsAlienWordleKey] as? Bool {
            // Assign the value of the setting to the `isAlienWordle` property
